...
void ngx_log_stderr(int err, const char *fmt, ...)
{
    ...
    p = ngx_cpymem(errstr, "urlsever: ", 10); //p指向"nginx: "之后
...
void ngx_log_init(int append)
{
    ...
    // cout<<&s<<endl;
    // plogname = (u_char *)p_config->get_item("log","error.log").c_str();    // 他是一个局部变量的返回值呀，返回之后就被释放了的呀，那么后面设置到会有哪些奇奇怪怪的内容呢，这些 就不确定了呀
    // cout<<&plogname<<"::"<<sizeof(*plogname)<<"::"<<plogname<<endl;
    // cout<<&s<<s<<endl;

    // string s = p_config->get_item("log", "error.log");
    // cout << s << endl;
    strcpy(plogname, p_config->get_item("log", "error.log").c_str());

    // cout<<&plogname<<":"<<plogname<<endl;

    // string path=p_config->get_item("PATH","h78z");
    // cout<<&s<<s<<endl;

    ngx_log.log_level = stoi(p_config->get_item("LogLevel", "8"));

    // 经过上面的那个函数之后，plogname会变成  h78z 这个字符串

    //  ！！c_str()函数的使用：https://www.cnblogs.com/cyx-b/p/12411673.html

    // cout<<&plogname<<":"<<plogname<<endl;
    // cout<<ngx_log.log_level<<endl;
    // exit(0);
    ...
    if (append)
        ...
        ngx_log.fd = open((const char *)plogname, O_WRONLY | O_TRUNC | O_CREAT, 0644);
